...
function Reply(props: PostReplyType) {
  ...
  const ReplyItem = ({ content, id, user, postCount, profileCount }: PostReplyType) => {
    return (
      <>
  ...
  return (
    <>
      <div className="flex flex-col py-2 border-t cursor-pointer border-base ">
        {props.replys.length ? RenderReplyList() : ReplyItem(props)}
      </div>
    </>
